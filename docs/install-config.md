...
| NetBox `manifest unknown` / image not found | Old Hub tag `netboxcommunity/netbox:v4.4-3.2.0` does not exist. Current pin is `ghcr.io/netbox-community/netbox:v4.6.9-5.0.2` (same bits as Hub `v4.6.9-5.0.2`; GHCR avoids Hub anonymous rate-limits that look like a missing tag). Redis stays `redis:7-alpine`. Image pins are in git `docker-compose.yml` — `config/forgesre.yml` is gitignored and does not change images. `git pull origin main && ./forgesre update` (not `./install.sh`). First NetBox pull is large and slow. |
