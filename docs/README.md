...
5. **Discovery vs NetBox** — `/discovery`. **Scan now** autodetects connected IPv4 networks (real prefixes, not a hardcoded `/24`), saves `discovery.cidrs`, probes TCP/SNMP/`/metrics` (not nmap). Candidate table: open ports, node_exporter, SNMP. **Sync NetBox** is half-width beside Scan now (**read-only** from bundled `:8001`). Empty NetBox is **yellow No devices** — that is normal. Approve or Ignore; nothing is auto-added. Handbook [§6.B](operator-handbook.md#b-discovery-scan-the-management-network) / [§6.C](operator-handbook.md#c-bundled-netbox-read-sync).
