...
Left nav is a constant dark shell (does not follow the theme). The control at the bottom cycles **Light / Dark / System**; System follows the OS for the main pane only. After a CSS change, hard-refresh so `/static/app.css` is not served from cache. Operator list tables (incidents, history, journal, mail outbox, reports, assets, discovery, playrules, admin users/audit) show **10 rows per page** with numbered tabs and Previous / Next.
