...
**Core is not a NetBox UI login.** `NETBOX_API_TOKEN` is a read-only **v1** token attached to the Django **superuser** (`NETBOX_SUPERUSER_NAME`, usually `admin`) with `is_superuser` and `dcim | device | Can view device`. ForgeSRE does not create a separate NetBox user named Core. A valid token on a user **without** that permission is still HTTP **403** on `GET /api/dcim/devices/`. Launch upserts the token onto that superuser on every NetBox start (`scripts/netbox-upsert-token.py`). `docker compose logs netbox | grep forgesre` should show **`v1 token ready`**. **`could not upsert`** means the token never reached the NetBox DB (UI still starts).
...
Core finds it via compose env `NETBOX_URL=http://127.0.0.1:8001` and `NETBOX_API_TOKEN` from `secrets/secrets.env` (Core reads the bind-mounted secrets file; do not interpolate an empty project `.env` over `env_file`). Discovery only mentions `--netbox-url` / an external instance when `inventory.netbox.url` is **not** localhost. The bundled NetBox container upserts that **plain** 40-character value on **every** start as a NetBox **v1** token (`plaintext` column, `write_enabled=False`, on `NETBOX_SUPERUSER_NAME`, permission to list devices) via `scripts/netbox-upsert-token.py`. Success: `docker compose logs netbox | grep forgesre` shows **`v1 token ready`**. **`could not upsert`** (UI still starts) means the token **never landed in the NetBox DB** — Discovery stays HTTP 403 and that is honest. The helper logs the Python exception type and message (never the token). NetBox 4.5+ has no `User.is_staff`; touching it was why upsert failed on a live v4.6 database. NetBox v4.6 defaults to hashed **v2** tokens (`key` is a 12-character public id + HMAC digest); writing the secret into `key` is why `GET /api/dcim/devices/` stayed HTTP **403**. netbox-docker 5.0.2 also skips `SUPERUSER_API_TOKEN` unless `SUPERUSER_API_KEY` is set, and first-insert-only superuser creation does not help an existing database. After `git pull origin main && ./forgesre update` (recreates `netbox` so launch runs), Core sync is GET-only. On **Discovery**, the NetBox light is a non-clickable status chip (`role=status`, CSS color, not a `<button>`; **Sync NetBox** is the only action) from `GET /api/dcim/devices/` (not a write): **grey** **Not connected** / **API 403** (UI down, API 403, no token); **yellow** **No devices** (API 200 and count == 0; empty is normal). Sentence: *No devices yet; add in NetBox UI `:8001` or use Assets/Discovery.* **green** **Connected** (API 200 and count ≥ 1). Empty must not look like 403. HTTP 403 with `API token: yes` means NetBox rejected the token Core already has (often a 12-character key prefix, or core not recreated after `secrets.env`) — recreate **netbox+core**; `API token: no` means the token is empty. Admin **Sync NetBox** is clickable on yellow and green; grey still allows a retry click when the UI answers (`/login/` / `/api/status/`) — a devices 403 is a warning, not `disabled`. First-boot (UI not answering) stays disabled with one sentence. Engineers see a disabled CTA and **Admin only.** Devices become local assets (`source=netbox`). Core **never writes** back to NetBox. Local inventory stays the monitoring source of truth. Do **not** re-run `./install.sh` (that regenerates secrets).

You do **not** need a second API token from the NetBox UI (`:8001`) for Core sync, and you do **not** create a Core user in NetBox. Do not paste a UI-created token into `secrets.env` — that is a different string from `NETBOX_API_TOKEN`. If you still create a token in the UI for yourself, NetBox v4.5+ needs `API_TOKEN_PEPPERS` (image env `API_TOKEN_PEPPER_1`). `./forgesre update` writes `NETBOX_API_TOKEN_PEPPER` once into `secrets/secrets.env` (and `.env`) if it is missing. `SECRET_KEY` for NetBox is `NETBOX_SECRET_KEY` (already generated). Do not drop database `forgesre`.
