...
`docker compose up -d` / `./forgesre update` starts NetBox with **no compose profile**. Image pin: `ghcr.io/netbox-community/netbox:v4.6.9-5.0.2` (same digest as Docker Hub `netboxcommunity/netbox:v4.6.9-5.0.2`; the old Hub tag `v4.4-3.2.0` does not exist). Redis for NetBox is `redis:7-alpine` on `:6379`. UI: `http://<VM-IP>:8001`. First login: user `admin`, email `admin@forgesre.local`, password `NETBOX_SUPERUSER_PASSWORD` in `secrets/secrets.env` (not an operator personal email). First boot pulls a large image then runs Django migrations and can take several minutes — **System Health** / `./forgesre doctor` stays **yellow** until `http://127.0.0.1:8001/login/` answers. That is not a fake green. Compose image pins live in git (`docker-compose.yml`); live `config/forgesre.yml` is gitignored and does not set image tags.
