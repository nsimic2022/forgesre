...
Pytest count is recorded after the double run on this branch (see the commit that says “Record double pytest”). Python 3.12, pytest 9.x. Ignore the Starlette `httpx` / `starlette.testclient` deprecation warning.
