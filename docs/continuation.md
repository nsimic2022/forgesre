...
Pytest count after the double run on `cursor/discovery-confirm-cidr-cb97`: **444 passed** (twice). Final `--no-ff` merge to `main`: **`3fa9e9a`**. `create_pr` / `ManagePullRequest` often **403** — `git merge --no-ff` plus `git push origin main` still lands the change.
...
SHA: **`3fa9e9a`**. Open **Discovery**. Review suggested `/24`, **Confirm & scan**, wait for the background job (`./forgesre jobs`). **Sync NetBox** sits beside Scan now (read-only). Approve / Ignore as before; manual Assets stay SoT.
