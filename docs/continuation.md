...
Pytest count after the double run on `cursor/discovery-confirm-cidr-cb97`: **444 passed** (twice). Final `--no-ff` merge to `main`: **`872bd41`**. `create_pr` / `ManagePullRequest` often **403** — `git merge --no-ff` plus `git push origin main` still lands the change.
