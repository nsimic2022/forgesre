...
- There is **no browser PTY**. A web terminal wrapped around `./forgesre` would still be a host command channel (XSS / stolen admin cookie). Use SSH + `./forgesre` / `./forgesre shell` on the box. See `docs/operator-handbook.md` (Administration → ForgeSRE CLI).
