...
Options:
  ...
  --profile standard|full-ai   full-ai downloads ~1.1 GB GGUF (not stored in git)
  --timezone ZONE
  --data-dir PATH
  --port N
...
wizard() {
  ...
  explain "AI / LLM" "Read-only wording on ForgeRCA. Downloads ~1.1 GB GGUF to data/models/ (not git)." "no" "Incidents and ForgeRCA still work without a model."
  if [[ "$PROFILE" == "full-ai" ]]; then ENABLE_AI="yes"; fi
  echo "Enable local LLM (downloads Qwen2.5-1.5B Instruct Q4_K_M, ~1.1 GB)?"
  echo "  1) Yes"
  echo "  2) No"
  ...
  explain "Discovery" "TCP probe of 22/80/443/9100/9182 plus SNMP GET on UDP/161, then HTTP /metrics on :9182/:9100 to pick Windows vs Linux. New hosts wait for Approve/Ignore. Not nmap." "no" "Inventory stays manual plus optional NetBox."
...
echo "Health lights:   ./forgesre doctor"
echo "Full report:     ./forgesre test"
